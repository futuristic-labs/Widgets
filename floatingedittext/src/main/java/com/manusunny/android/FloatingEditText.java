...
import com.manusunny.android.floatingedittext.R;
...
public class FloatingEditText extends EditText {
    ...
    public FloatingEditText(final Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        Drawable drawable = new Drawable() {
            ...
            public void draw(Canvas canvas) {
                if (mVerified) {
                    ...
                    float y = rect.bottom + (dpToPx(context, 16) - mHintPaint.getFontMetricsInt().top) / 2;
        ...
        int paddingTop = dpToPx(context, 12);
        int paddingBottom = dpToPx(context, 20);
        setPadding(0, paddingTop, 0, paddingBottom);
    }

    private Rect getThinLineRect(Canvas canvas) {
        lineRect.left = getPaddingLeft();
        lineRect.top = canvas.getHeight() - mUnderlineHeight - dpToPx(context, 16);
        lineRect.right = getWidth();
        lineRect.bottom = canvas.getHeight() - dpToPx(context, 16);
        return lineRect;
    }

    private Rect getThickLineRect(Canvas canvas) {
        lineRect.left = getPaddingLeft();
        lineRect.top = canvas.getHeight() - mUnderlineHighlightedHeight - dpToPx(context, 16);
        lineRect.right = getWidth();
        lineRect.bottom = canvas.getHeight() - dpToPx(context, 16);
        return lineRect;
    }
    ...
    public void setValidationResult(boolean verified, String message) {
    ...
    protected void onDraw(@NonNull Canvas canvas) {
        ...
        if (!TextUtils.isEmpty(getHint())) {
            ...
            float minHintY = getBaseline() + getPaint().getFontMetricsInt().top + getScrollY() - dpToPx(context, 4);
            float textSize;
            float hintY;
